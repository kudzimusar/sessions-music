...
const authHeaders={'oai-authenticated-user-email':'worker-fixture@example.test'};
after(()=>runtime.dispose());

test('anonymous protected surfaces fail closed into onboarding without leaking marketplace data',async()=>{
...
test('authenticated production Worker opens on canonical Sessions customer home while sourced inventory remains canonical',async()=>{
...
test('authenticated production Worker renders canonical mobile home, map and real studio details',async()=>{
...
test('authenticated production Worker exposes planner, registration, capacity and visible payment states',async()=>{
 for(const [path,pattern] of [['/planner',/Guided planner/],['/register',/My studio is missing/],['/subscriptions',/Price not set/],['/onboarding/onevibe-studiox',/Sign in to continue|OneVibe Studiox/],['/studio/onevibe-studiox',/Capacity not supplied/]]){const r=await runtime.dispatchFetch('https://sessions.test'+path,{headers:authHeaders});assert.equal(r.status,200);assert.match(await r.text(),pattern);}
...
test('authenticated studio sharing metadata matches visible records and never inherits generic studio imagery',async()=>{
