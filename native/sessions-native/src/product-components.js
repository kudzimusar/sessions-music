...
export function StudioArtwork({studio,compact=false}){
  return <View style={[styles.art,compact&&styles.artCompact]} accessibilityLabel={`${studio.name} image placeholder`}><Between><Pill>{studio.verified?'Verified':'Profile'}</Pill>{studio.backupPower?<Text style={styles.power}>Backup power</Text>:null}</Between><Text style={[styles.initials,compact&&styles.initialsCompact]}>{initials(studio.name)}</Text></View>;
}
...
const styles=StyleSheet.create({
  ...
  art:{height:145,backgroundColor:'#111827',padding:14,justifyContent:'space-between'},artCompact:{height:120},initials:{fontSize:40,fontWeight:'900',color:c.white,letterSpacing:-1.6},initialsCompact:{fontSize:34},power:{fontSize:10,fontWeight:'800',color:c.white},
