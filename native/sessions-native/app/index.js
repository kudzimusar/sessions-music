...
import {readAuthConfig,SessionsApiError} from '../src/api';
...
export default function GatewayScreen(){
  ...
  useEffect(()=>{let active=true;readAuthConfig().then(value=>{if(active)setConfig(value)}).catch(()=>{if(active)setConfig(null)});return()=>{active=false}},[]);
  ...
  return <Screen offline={!online} contentStyle={{paddingTop:28}}><Eyebrow>SESSIONS · NATIVE GATEWAY</Eyebrow><Title>One identity. Your music spaces.</Title><Body style={{marginTop:10}}>Sign in to discover, book and manage Sessions. Customer is the default context; provider access is earned through a verified studio relationship.</Body>
    ...
    <Card><Eyebrow>AUTH BOUNDARY</Eyebrow><Small style={{marginTop:6}}>Authority: {NATIVE_AUTH_BOUNDARY.provider}</Small><Small>Status: {NATIVE_AUTH_BOUNDARY.status}</Small><Small>Server auth config: {config?'reachable':'not certified / unavailable'}</Small></Card>
