...
export default function GatewayScreen(){
  ...
  const explainFederated=()=>Alert.alert('Google / Apple sign-in is not certified yet','Use native phone or email verification for now. Federated sign-in will only be enabled after the verified Sessions Supabase project exposes the provider and the platform callback flow is tested end to end.');
  return <Screen offline={!online} contentStyle={{paddingTop:28}}><Eyebrow>SESSIONS · NATIVE GATEWAY</Eyebrow><Title>One identity. Your music spaces.</Title><Body style={{marginTop:10}}>Sign in to discover, book and manage Sessions. Customer is the default context; provider access is earned through a verified studio relationship.</Body>
    <View style={{marginTop:26,gap:10}}><PrimaryButton onPress={()=>router.push(`/sign-in?continue=${encodeURIComponent(continuation)}`)}>Continue with phone or email</PrimaryButton><SecondaryButton onPress={explainFederated}>Continue with Google / Apple</SecondaryButton></View>
    ...
    <Card style={{marginTop:24}}><Eyebrow>NATIVE UAT REVIEW</Eyebrow><Body style={{marginTop:6}}>This build contains a clearly isolated fixture projection so the iOS/Android interaction model can be reviewed while live native identity activation is being certified. Fixture actions never create production authority or authenticated mutations.</Body><View style={{marginTop:14}}><PrimaryButton onPress={()=>router.replace(continuation)}>Open UAT app preview</PrimaryButton></View></Card>
    <Card><Eyebrow>AUTH BOUNDARY</Eyebrow><Small style={{marginTop:6}}>Authority: {NATIVE_AUTH_BOUNDARY.provider}</Small><Small>Status: {NATIVE_AUTH_BOUNDARY.status}</Small><Small>Verified project: {NATIVE_AUTH_BOUNDARY.projectRef}</Small><Small>Server auth config: {Platform.OS==='web'?'not queried in browser UAT':config?.enabled?'enabled':config?'present but disabled':'unavailable'}</Small></Card>
