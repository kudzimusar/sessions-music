...
export function CheckField({checked,onChange,children,disabled=false}:{checked:boolean;onChange:(v:boolean)=>void;children:ReactNode;disabled?:boolean}){return <label className="r-check"><Checkbox checked={checked} disabled={disabled} onCheckedChange={v=>onChange(v===true)}/><span>{children}</span></label>}
