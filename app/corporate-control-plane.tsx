...
import {Activity,BarChart3,BookOpenCheck,Building2,RefreshCw,ShieldCheck,UsersRound} from 'lucide-react';
...
export default function CorporateControlPlane({module}:{module:CorporateReadModule}){
 ...
 return <section className="r-width r-inner c4-module">
  <div className="r-workspace-heading"><div><div className="r-eyebrow"><Icon size={16}/>SESSIONS CORPORATE CONTROL PLANE</div><h1>{data.title}</h1><p className="r-intro">{data.description}</p><div className="c4-ownership"><span><Building2 size={15}/><strong>Owner:</strong> {definition.ownerDepartment}</span>{definition.supportingDepartments?.length?<span><strong>Supporting:</strong> {definition.supportingDepartments.join(' · ')}</span>:null}</div></div><div className="r-panel c4-source-card"><strong>Canonical projection</strong><p>This screen reads the existing marketplace source of truth. It does not create a corporate copy of the domain.</p></div></div>
