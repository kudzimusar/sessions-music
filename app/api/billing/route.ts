...
export async function GET(req:Request){try{
 const config=billingConfiguration(),user=await getProductionUser(),studioId=new URL(req.url).searchParams.get('studio');if(!user||!studioId)return reply({...config,account:null,checkouts:[]});
 const owner=await database().prepare('SELECT owner FROM studio_registry WHERE id=?').bind(studioId).first();if(owner?.owner!==user.id)return reply({...config,account:null,checkouts:[]});
...
export async function POST(req:Request){try{
 ...
 const user=await getProductionUser();if(!user)return reply({error:'Sign in to manage studio subscriptions'},401);const raw=await req.text();if(raw.length>3000)return reply({error:'Request too large'},413);
 ...
 const owner=await database().prepare('SELECT owner FROM studio_registry WHERE id=?').bind(p.studioId).first();if(owner?.owner!==user.id)return reply({error:'Only the verified studio owner can manage its billing'},403);
 if(p.action==='checkout'){if(!p.id||!p.provider||!p.consent)return reply({error:'Review and accept the subscription terms first'},400);return reply(await startCheckout(p.studioId,user.id,p.provider,p.id));}
