...
async function reportingCycle(db:any,staffId:string,managerStaffId:string){
 ...
 const parent=new Map(rows.map(row=>[String(row.staff_id),String(row.manager_staff_id)]));parent.set(staffId,managerStaffId);let cursor=managerStaffId;const seen=new Set<string>();
 while(cursor){if(cursor===staffId)return true;if(seen.has(cursor))return true;seen.add(cursor);cursor=parent.get(cursor)||''}return false;
}
...
export async function POST(request:Request){
 try{
  ...
  const input=mutation.parse(await request.json());const db=database();const createdAt=now();let entityId='';let event='';let statement:any;
  if(input.action==='createDepartment'){
   ...
   entityId=newId('dept');event='department.created';statement=db.prepare('INSERT INTO corporate_departments(id,code,name,parent_department_id,status,created_at,updated_at,content) VALUES(?,?,?,?,?,?,?,?)').bind(entityId,input.code,input.name,input.parentDepartmentId,'active',createdAt,createdAt,JSON.stringify({description:input.description}));
   ...
   entityId=newId('pos');event='position.created';statement=db.prepare('INSERT INTO corporate_positions(id,department_id,code,title,level,reports_to_position_id,is_department_head,status,created_at,updated_at,content) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(entityId,input.departmentId,input.code,input.title,input.level,input.reportsToPositionId,input.isDepartmentHead?1:0,'active',createdAt,createdAt,JSON.stringify({description:input.description}));
   ...
   entityId=newId('staff');event='staff.invited';statement=db.prepare('INSERT INTO corporate_staff(id,user_id,staff_code,position_id,status,started_at,ended_at,created_at,updated_at,content) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(entityId,input.userId,input.staffCode,input.positionId,'invited',null,null,createdAt,createdAt,JSON.stringify({displayName:input.displayName,workEmail:input.workEmail,location:input.location,employmentType:input.employmentType,plannedStartAt:input.startedAt||null}));
   ...
   entityId=newId('report');event='reporting_line.created';const effectiveFrom=input.effectiveFrom||createdAt.slice(0,10);const statements=[] as any[];
   ...
   entityId=newId('deleg');event='delegation.created';const status=Date.parse(input.startsAt)>Date.now()?'scheduled':'active';statement=db.prepare('INSERT INTO corporate_delegations(id,principal_staff_id,delegate_staff_id,scope,status,starts_at,ends_at,created_by,created_at,revoked_at,content) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(entityId,input.principalStaffId,input.delegateStaffId,input.scope,status,input.startsAt,input.endsAt,actor.id,createdAt,null,JSON.stringify({reason:input.reason}));
  }
  ...
  const message=error instanceof Error?error.message:'Unknown error';if(/UNIQUE constraint failed/i.test(message))return response({error:'That organization code or staff identity is already in use.'},409);
  console.error('Corporate organization mutation failed',message);return response({error:'Corporate organization change could not be completed.'},503);
 }
}
