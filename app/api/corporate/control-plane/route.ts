...
async function audit(db:any,includeOrganization:boolean,includeSecurity:boolean){
 ...
 return {metrics:[metric('Marketplace events',marketplace.length),metric('Organization events',organization.length),metric('Security events',security.length)],columns:['Source','Event','Actor ref','Target','Time'].map((value,index)=>index===2?'Target':index===3?'Actor ref':value),rows:merged.map(item=>item.values),note:includeSecurity?'Security events are visible because this account also has Super Administration authority. Actor references are masked; event content payloads and identity-session IDs are intentionally not projected.':'Security-event payloads remain outside this audit projection; Super Administration authority is required to include the security stream. Actor references are masked.'};
}
