...
export async function POST(request:Request){
 ...
 try{
  ...
  if(purpose&&!['studio_room_photo','studio_verification'].includes(purpose))return Response.json({error:'Unsupported upload purpose'},{status:400});
  const db=database();
  if(purpose){
  ...
  await db.prepare('INSERT INTO uploads(id,owner,type,studio_id,room_id,purpose,created_at) VALUES(?,?,?,?,?,?,?)').bind(objectId,user.id,file.type,studioId||null,roomId||null,purpose||null,new Date().toISOString()).run();
