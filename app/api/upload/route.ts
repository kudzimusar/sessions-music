...
export async function POST(request:Request){
 ...
 try{
  ...
  if(purpose&&!['studio_room_photo','studio_verification','settlement_proof','booking_message_attachment','case_evidence'].includes(purpose))return Response.json({error:'Unsupported upload purpose'},{status:400});
  const db=database();
  if(purpose){
   if(purpose==='case_evidence'){
    if(!hasPermission(user,'cases:restricted.read'))return Response.json({error:'Restricted case evidence authority is required.'},{status:403});
    const total=await db.prepare("SELECT count(*) n FROM uploads WHERE owner=? AND purpose='case_evidence'").bind(user.id).first();if(Number(total?.n||0)>=100)return Response.json({error:'Case evidence upload limit reached.'},{status:429});
   }else{
