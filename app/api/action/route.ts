...
const photoSchema=z.string().regex(/^\/(images\/(studio|hall|intimate)\.webp|api\/media\/[a-zA-Z0-9-]+)$/);
const roomSchema=z.object({id:identifier,name:text,venue:text,category:z.enum(['Studio','Church','School','University','Arts centre','Institution','Private approved facility']),area:z.enum(AREAS as [string,...string[]]),description:z.string().min(15).max(3000),price:z.number().int().min(100).max(100000),capacity:z.number().int().min(1).max(200),equipment:z.array(z.enum(EQUIPMENT)).max(9),backup:z.boolean(),parking:z.boolean(),accessible:z.boolean(),instant:z.boolean(),active:z.boolean(),image:photoSchema,photos:z.array(photoSchema).max(5).optional(),minimum:z.number().int().min(30).max(480).multipleOf(30),deposit:z.number().int().min(0).max(100000),buffer:z.number().int().min(0).max(120).multipleOf(30),cancellation:z.union([z.literal(24),z.literal(48)]),hours:z.record(z.tuple([z.number().int().min(0).max(1440).multipleOf(30),z.number().int().min(0).max(1440).multipleOf(30)]).nullable()),rules:z.string().min(5).max(3000),address:text,contact:z.string().max(200),checkin:z.string().max(1000),restricted:z.string().max(1000)});
