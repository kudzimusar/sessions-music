...
import {createInAppNotification,studioInboxRecipient} from '@/lib/booking-communications';
...
async function access(user:SessionUser,studioId:string){
 ...
 if(hasPermission(user,'settlements:review'))return {studio,role:'finance' as const};
...
export async function GET(request:Request){
 try{
  ...
  }else if(isRegistryUserOperator(user)||hasPermission(user,'settlements:review')){rows=(await db.prepare('SELECT revision,content FROM studio_settlements ORDER BY updated_at DESC LIMIT 2000').all()).results;invoices=(await db.prepare('SELECT content FROM studio_invoices ORDER BY period DESC LIMIT 1000').all()).results;
...
export async function POST(request:Request){
 try{
  ...
  const row=await db.prepare('SELECT revision,content FROM studio_settlements WHERE id=? AND studio_id=?').bind(value.settlementId,studioId).first();if(!row)fail('Settlement not found',404);if(row.revision!==value.revision)fail('This settlement changed. Refresh and try again.',409);const settlement:StudioSettlement={...JSON.parse(row.content),revision:row.revision};const bookingRow=await db.prepare('SELECT content FROM studio_bookings WHERE id=? AND studio_id=?').bind(settlement.bookingId,studioId).first();if(!bookingRow)fail('Booking not found',404);const booking=JSON.parse(bookingRow.content);const isCustomer=settlement.customer===user.id,isStudioManager=allowed.role==='owner'||allowed.role==='manager';
  let event:string;
  if(type==='submitProof'){
   ...
   if(!hasPermission(user,'settlements:review')&&!isRegistryUserOperator(user))fail('Finance settlement-review authority required',403);if(settlement.status!=='disputed')fail('Only an open dispute can be resolved',409);const resolution=z.object({note:z.string().trim().min(20).max(1000)}).parse(raw);const previous=(settlement as StudioSettlement&{previousStatus?:StudioSettlement['status']}).previousStatus;if(!previous||previous==='disputed')fail('The prior settlement state is unavailable',409);Object.assign(settlement,{status:previous,resolutionNote:resolution.note,updatedAt:now});event='dispute_resolved';
  }
  const notices:{recipient:string;content:string}[]=[];
  if(event==='proof_submitted')notices.push({recipient:studioInboxRecipient(studioId),content:`Payment proof was submitted for ${booking.roomName} on ${booking.date}. Review it before confirming direct payment.`});
  if(event==='settled_off_platform')notices.push({recipient:booking.customer,content:`${booking.roomName} payment was recorded as received directly by the studio. Sessions did not move the money.`});
  if(event==='proof_declined')notices.push({recipient:booking.customer,content:'The studio declined the payment proof and left a reason. You can submit a corrected record or flag a dispute.'});
  if(event==='dispute_opened')notices.push({recipient:isCustomer?studioInboxRecipient(studioId):booking.customer,content:'A settlement issue was flagged for Finance review. The direct payment record is paused; Sessions cannot reverse money.'});
  if(event==='dispute_resolved')notices.push({recipient:booking.customer,content:'Sessions Finance recorded a settlement dispute resolution. Check the booking record for the current state.'},{recipient:studioInboxRecipient(studioId),content:'Sessions Finance recorded a settlement dispute resolution. Check the booking record for the current state.'});
  const notifications=notices.map(value=>createInAppNotification({booking,recipient:value.recipient,kind:'settlement_updated',content:value.content,now}));
  const nextRevision=settlement.revision+1;const guard=crypto.randomUUID();const serialized=JSON.stringify({...settlement,revision:nextRevision});await db.batch([
   ...
   ...notifications.map(notification=>db.prepare('INSERT INTO booking_notifications(id,booking_id,studio_id,recipient,channel,kind,status,idempotency_key,created_at,attempts,content) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(notification.id,booking.id,studioId,notification.recipient,notification.channel,notification.kind,notification.status,`settlement:${event}:${settlement.id}:${value.key}`,now,0,JSON.stringify(notification))),
