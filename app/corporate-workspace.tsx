...
type Office={permission:string;title:string;copy:string;icon:typeof ShieldCheck;workspace:'operations'|'role-control'|'segregated'};
const offices:Office[]=[
 {permission:'claims:review',title:'Trust & safety',copy:'Ownership claims, studio verification and marketplace integrity.',icon:ShieldCheck,workspace:'segregated'},
 {permission:'settlements:review',title:'Finance',copy:'Settlement exceptions, invoices, fee governance and reconciliation.',icon:ReceiptText,workspace:'segregated'},
 {permission:'support:read',title:'Customer support',copy:'Customer-facing issues and operational service health without private message access.',icon:Headphones,workspace:'segregated'},
 {permission:'providers:oversight',title:'Provider operations',copy:'Provider onboarding, registrations, verified status and marketplace readiness.',icon:Building2,workspace:'operations'},
 {permission:'platform:roles.manage',title:'Super administration',copy:'Platform authority and office-role assignment. No provider or customer can self-promote.',icon:UserCog,workspace:'role-control'},
];
...
export default function CorporateWorkspace(){
 ...
 return <section className="r-width r-inner">
  ...
  <h2>Corporate offices</h2><div className="r-studio-grid">{offices.map(({permission,title,copy,icon:Icon,workspace})=>{const allowed=permissions.has(permission);const operationLink=allowed&&workspace==='operations'&&canOpenOperations;return <article className="r-panel" key={permission}><Icon size={25}/><span className={'r-badge '+(allowed?'claimed':'')}>{allowed?'Assigned':'Not assigned'}</span><h3>{title}</h3><p>{copy}</p>{!allowed?<p className="r-small">This account cannot open this office.</p>:workspace==='role-control'?<a className="r-primary" href="#role-control">Manage authority</a>:operationLink?<a className="r-primary" href="/registry-admin">Open Operations workspace</a>:<p className="r-small">Authority active. This office remains segregated from the general Operations console; only its permitted metrics are shown here until its dedicated action queue is enabled.</p>}</article>})}</div>
