...
import {registrySlotReason} from './registry';
...
export function findAStudioNearMe(studios:Studio[],position:Coordinates,radius=25){return studios.filter(s=>!s.hidden&&s.location).map(studio=>({studio,distance:distanceKm(position,studio.location!)})).filter(s=>s.distance<=radius).sort((a,b)=>a.distance-b.distance);}
