...
export function parseSearch(text:string,date=localDate()):SearchInterpretation{
 ...
 const bookingMode:SearchInterpretation['bookingMode']=/approval|request to book|needs? approval/.test(t)?'approval':/instant|book now|immediate/.test(t)?'instant':'any';
