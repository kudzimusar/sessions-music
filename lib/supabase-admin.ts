...
function config(){
 ...
 if(!url||!secret)throw new Error('Supabase server provisioning is not configured. Add the secret key to the Site runtime before approving owners or staff.');
 return {url,secret};
}
...
async function request<T>(path:string,init:RequestInit):Promise<T>{
 ...
 if(!response.ok){const detail=(await response.text()).slice(0,500);throw new Error(`Supabase membership provisioning failed (${response.status})${detail?`: ${detail}`:''}`)}
