...
import {studioSeeds,kulchaAddressCorrection,type Studio,type Staff,type RegistryState,type StudioBooking,type Claim,type RegistryIssue,type RegistryUser,type VerificationRequest} from '@/lib/registry';
...
export async function readRegistry(user:(Partial<RegistryUser>&{email?:string|null;displayName:string;memberships?:readonly {organizationId:string;active:boolean}[]})|null,studioId?:string):Promise<RegistryState>{
 ...
 const bookings:StudioBooking[]=actor?(await db.prepare('SELECT content FROM studio_bookings WHERE customer=? OR studio_id IN (SELECT id FROM studio_registry WHERE owner=?) OR studio_id IN (SELECT studio_id FROM studio_staff WHERE email IN (?,?) AND status=\'active\' AND role=\'manager\') ORDER BY rowid DESC LIMIT 1000').bind(actor,actor,contacts[0]||'',contacts[1]||'').all()).results.map((r:any)=>JSON.parse(r.content)):[];
 ...
 return {memberships,registrations,verifications,studios,staff,managedIds,ownerIds,bookings,claims,issues,operator,user:normalizedUser,invitations:personalStaff.filter((s:Staff)=>s.status==='invited'),myStaff:personalStaff.filter((s:Staff)=>s.status==='active'),occupancy} as RegistryState;
}
